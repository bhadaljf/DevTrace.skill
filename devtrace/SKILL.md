...
DevTrace 维护单一技能正文，同时适配 Claude Code 与 Codex。

- Claude Code：建议放在 `.claude/skills/devtrace/`，通常可通过 `/devtrace` 调用
- Codex：建议放在 `~/.codex/skills/devtrace/` 作为 skill 使用；如需插件化分发，可再使用 `.codex-plugin/plugin.json`
- 为保持双平台兼容，正文中的操作描述尽量使用“使用 DevTrace ...”这一平台无关表述，不把命令格式写死
